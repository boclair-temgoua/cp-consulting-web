...
import { CategoriesCreateFormModal } from './hook/CategoriesCreateFormModal';
import { getCategoriesBy } from './core/_requests'
import { CategoryModel } from './core/_models'
import CategoriesList from './hook/CategoriesList'
...
const CategoriesWrapper: FC = () => {
  ...
  const {
    ...
    data,
    isPreviousData,
  } = useQuery(['categories', pageItem, 'DESC', debouncedFilter], () => fetchData(pageItem, debouncedFilter), {
  ...
  const dataTable = isLoading ? (<tr><td><strong>Loading...</strong></td></tr>) :
    isError ? (<tr><td><strong>Error find data please try again...</strong></td></tr>) :
      (data?.data?.total <= 0) ? (<EmptyTable name='categories' />) :
        (
          data?.data?.value?.map((item: CategoryModel, index: number) => (
            <CategoriesList item={item} key={index} roleItem={userItem?.role} />
          )))
  ...
  return (
    <>
      ...
      <PageTitle breadcrumbs={[{
        title: `${userItem?.organization?.name || 'Categories'} |`,
        path: '/categories',
        isSeparator: false,
        isActive: false,
      ...
      <div className={`card mb-5 mb-xl-8`}>
        ...
        <div className='card-header border-0 pt-5'>
          <h3 className='card-title align-items-start flex-column'>
            ...
            <span className='text-muted mt-1 fw-semibold fs-7'>Over {data?.data?.total || 0} categories</span>
          </h3>
          <div
        ...
        <div className='card-body py-3'>
          ...
          {openCreateOrUpdateModal && (<CategoriesCreateFormModal setOpenCreateOrUpdateModal={setOpenCreateOrUpdateModal} />)}
          {/* end::Table container */}
        </div>
        {/* begin::Body */}
      </div>
