...
import { getContributorsSubSubProject } from './core/_requests';
...
const ContributorSubSubProjectTableMini: React.FC<Props> = ({ subSubProject, takeValue }) => {
    const [openModal, setOpenModal] = useState<boolean>(false)

    const fetchDataContributor = async () => await getContributorsSubSubProject({ take: takeValue, page: 1, sort: 'DESC', subSubProjectId: String(subSubProject?.id) })
    const { isLoading: isLoadingContributor, isError: isErrorContributor, data: dataContributor } = useQuery({
        queryKey: ['contributors', subSubProject?.id],
            ...
            (dataContributor?.data?.total <= 0) ? (<EmptyTable name='contributor' />) :
                (
                    dataContributor?.data?.value?.map((item: ContributorModel, index: number) => (
                        <ContributorList item={item} key={index} contributor={subSubProject} />
                    )))
    ...
    return (
        <>
            ...
            {openModal && (<InviteContributorFormModal setOpenModal={setOpenModal} subSubProjectId={subSubProject?.id} />)}

        </>
    )
}

export { ContributorSubSubProjectTableMini }
